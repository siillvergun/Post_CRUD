...
public class MemoryUserRepository implements UserRepository {
    ...
    private static Map<Long, UserEntity> store = new HashMap<>();
    ...
    public UserEntity save(UserEntity userEntity) {
        ...
        userEntity.assignUserId(++seq);
        // 저장하려는 유저를 put 메서드를 사용해 저장 후 저장된 유저 반환
        store.put(userEntity.getUserId(), userEntity);
        return userEntity;
    }
    ...
    public Optional<UserEntity> findByEmail(String email) {
        return store.values().stream().filter(entity -> entity.getEmail().equals(email)).findAny();
    }

    @Override
    public Optional<UserEntity> findByNickname(String nickname) {
        return store.values().stream().filter(entity -> entity.getNickname().equals(nickname)).findAny();
    }
